...
struct ClipboardItem: Identifiable, Codable {
    ...
    init(imageData: Data) {
        ...
        do {
            ...
            let fileURL = try ClipboardManager.ensureClipboardDataDirectory().appendingPathComponent(fileName)
...
class ClipboardManager: ObservableObject {
    ...
    // Directory for storing clipboard data files. Created only when clipboard history is explicitly enabled and used.
    static var clipboardDataDirectory: URL {
        let documentsPath = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        return documentsPath.appendingPathComponent("ClipboardData")
    }

    static func ensureClipboardDataDirectory() throws -> URL {
        let dir = clipboardDataDirectory
    ...
    func startMonitoring() {
        guard Defaults[.enableClipboardManager] else {
            print("🔒 ClipboardManager: monitoring blocked because clipboard history is disabled")
            return
        }
        guard !isMonitoring else { return }

        if clipboardHistory.isEmpty && pinnedItems.isEmpty {
            loadHistoryFromDefaults()
        }
        lastChangeCount = NSPasteboard.general.changeCount
        isMonitoring = true
    ...
    func clearAllData() {
        ...
        clearHistory()
        clearPinnedItems()
        try? FileManager.default.removeItem(at: ClipboardManager.clipboardDataDirectory)
    }
    ...
    private func cleanupOldFiles() {
        guard FileManager.default.fileExists(atPath: ClipboardManager.clipboardDataDirectory.path) else { return }

...
class PrivacyIndicatorManager: ObservableObject {
    ...
    var indicatorLayout: IndicatorLayout {
        ...
        let recording = screenRecordingActive && Defaults[.enableScreenRecordingDetection]
    ...
    var hasAnyIndicator: Bool {
        ...
        let showRecording = screenRecordingActive && Defaults[.enableScreenRecordingDetection]
        return showCamera || showMic || showRecording
    }
    ...
    func startMonitoring() {
        guard Defaults[.enableCameraDetection] || Defaults[.enableMicrophoneDetection] else {
            print("PrivacyIndicatorManager: start blocked because camera and microphone detection are disabled")
            return
        }
        ...
        if Defaults[.enableCameraDetection], cameraMonitor.isMonitoringAvailable {
            cameraMonitor.startMonitoring()
        } else if Defaults[.enableCameraDetection] {
        ...
        if Defaults[.enableMicrophoneDetection], microphoneMonitor.isMonitoringAvailable {
            microphoneMonitor.startMonitoring()
        } else if Defaults[.enableMicrophoneDetection] {

...
private actor MusicExplicitnessResolver {
    ...
    private static func fetchExplicitness(for key: LookupKey, using session: URLSession) async -> Bool {
        ...
        do {
            let (data, _) = try await PrivacyNetworkGate.data(from: url, using: session, feature: .musicMetadata)
...
private actor SpotifyExplicitnessResolver {
    ...
    private static func fetchExplicitness(for key: LookupKey, using session: URLSession) async -> Bool? {
        ...
        do {
            let (data, response) = try await PrivacyNetworkGate.data(from: url, using: session, feature: .musicMetadata)
...
class MusicManager: ObservableObject {
    ...
    private func updateFromPlaybackState(_ state: PlaybackState) {
        let shouldAutoPeekOnPlaybackChange = Defaults[.showSneakPeekOnTrackChange]
        ...
        if eventIsPlaying != self.isPlaying {
            ...
            if shouldAutoPeekOnPlaybackChange && eventIsPlaying && !state.title.isEmpty && !state.artist.isEmpty {
        ...
        if hasContentChange {
            ...
            if shouldAutoPeekOnPlaybackChange && !state.title.isEmpty && !state.artist.isEmpty && state.isPlaying {
                self.updateSneakPeek()
            }
    ...
    private func fetchLyricsFromAPI(artist: String, title: String) async throws -> [LyricLine] {
        ...
        let (data, response) = try await PrivacyNetworkGate.data(from: url, feature: .lyrics)

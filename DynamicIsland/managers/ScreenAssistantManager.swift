...
class ScreenAssistantManager: NSObject, ObservableObject {
    ...
    // Directory for storing audio recordings. Created only when recording is explicitly used.
    static var audioDataDirectory: URL {
        let documentsPath = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        return documentsPath.appendingPathComponent("ScreenAssistantAudio")
    }

    static func ensureAudioDataDirectory() throws -> URL {
        let dir = audioDataDirectory
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        return dir
    }

    // Directory for storing screenshots. Created only when screenshots are explicitly used.
    static var screenshotDataDirectory: URL {
        let documentsPath = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        return documentsPath.appendingPathComponent("ScreenAssistantScreenshots")
    }

    static func ensureScreenshotDataDirectory() throws -> URL {
        let dir = screenshotDataDirectory
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        return dir
    }
    ...
    func addFiles(_ urls: [URL]) {
        guard Defaults[.enableScreenAssistant] else {
            print("🔒 ScreenAssistant: addFiles blocked because Screen Assistant is disabled")
            return
        }
    ...
    func toggleRecording() {
        guard Defaults[.enableScreenAssistant] else {
            print("🔒 ScreenAssistant: recording blocked because Screen Assistant is disabled")
            return
        }
    ...
    private func startRecording() {
        ...
        do {
            audioURL = try ScreenAssistantManager.ensureAudioDataDirectory().appendingPathComponent(fileName)
        } catch {
            print("Failed to create Screen Assistant audio directory: \(error)")
            return
        }
    ...
    func sendMessage(_ message: String) {
        guard Defaults[.enableScreenAssistant] else {
            print("🔒 ScreenAssistant: message blocked because Screen Assistant is disabled")
            return
        }
    ...
    private func sendToAI(message: String, files: [ScreenAssistantFile], provider: AIModelProvider) {
        guard provider == .local || Defaults[.enableExternalAIProviders] else {
            addAssistantMessage("External AI providers are disabled in privacy-safe mode. Enable external AI and add an API key before sending data to a cloud provider.")
            isLoading = false
            return
        }
    ...
    private func performAPIRequest(url: URL, requestBody: [String: Any], provider: AIModelProvider) {
        ...
        let feature: PrivacyNetworkFeature = provider == .local ? .localAI : .externalAI
        var task: URLSessionDataTask?
        do {
            task = try PrivacyNetworkGate.dataTask(with: request, feature: feature) { [weak self] data, response, error in
            ...
            print("🔒 ScreenAssistant: request blocked - \(error.localizedDescription)")
            addAssistantMessage(error.localizedDescription)
            isLoading = false
            return
        }
    ...
    private func performOpenAIRequest(url: URL, requestBody: [String: Any], apiKey: String, provider: AIModelProvider = .openai) {
        ...
        var task: URLSessionDataTask?
        do {
            task = try PrivacyNetworkGate.dataTask(with: request, feature: .externalAI) { [weak self] data, response, error in
                DispatchQueue.main.async {
                    guard let self = self else { return }
                    guard let currentTask = task else { return }
    ...
    func clearSensitiveLocalData() {
        stopRecording()
        resetConversationContext()
        UserDefaults.standard.removeObject(forKey: "ScreenAssistantFiles")
        try? FileManager.default.removeItem(at: Self.audioDataDirectory)
        try? FileManager.default.removeItem(at: Self.screenshotDataDirectory)
    }

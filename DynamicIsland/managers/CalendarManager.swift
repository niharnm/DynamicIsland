...
class CalendarManager: ObservableObject {
    ...
    private func handleEventStoreChanged() {
        guard calendarFeaturesEnabled else { return }
    ...
    func updateLockScreenEvents(force: Bool = false) async {
        guard Defaults[.lockScreenShowCalendarEvent] || Defaults[.enableLockScreenReminderWidget] else {
            lockScreenEvents.removeAll()
            return
        }
    ...
    private func startLockScreenRefreshLoop() {
        ...
        lockScreenRefreshTask = Task { [weak self] in
            ...
            while !Task.isCancelled {
                ...
                guard self.hasCalendarAccess, self.calendarFeaturesEnabled else { continue }
    ...
    private func updateEvents(force: Bool = false) async {
        guard Defaults[.showCalendar] || Defaults[.enableReminderLiveActivity] else {
            events.removeAll()
            return
        }

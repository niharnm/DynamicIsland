...
final class LocalSendService: NSObject, ObservableObject {
    ...
    func startDiscovery() {
        guard Defaults[.quickShareProvider] == "LocalSend" else {
            Logger.log("LocalSend discovery blocked because LocalSend is not selected", category: .extensions)
            return
        }
    ...
    private nonisolated static func probeDeviceInfo(at ip: String, port: Int, timeout: TimeInterval) async -> LocalSendDeviceInfo? {
        ...
        for scheme in schemes {
            for path in paths {
                ...
                do {
                    let (data, response) = try await PrivacyNetworkGate.data(for: request, using: probeSession, feature: .localSend)
    ...
    private func sendAnnouncement() {
        let payload: [String: Any] = [
            "alias": Host.current().localizedName ?? "DynamicIsland",
            "version": "2.1",
            "deviceModel": "Mac",
            "deviceType": "desktop",
            "fingerprint": "atoll.localsend.bridge",
    ...
    private func prepareUpload(payload: [String: Any], baseURL: String) async throws -> (sessionID: String, fileTokens: [String: String]) {
        ...
        let (data, response) = try await PrivacyNetworkGate.data(for: request, using: trustedSession, feature: .localSend)
    ...
    private func upload(file: TransferFile, sessionID: String, token: String, baseURL: String, progress: @escaping (Double) -> Void) async throws {
        ...
        let (data, response) = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<(Data, URLResponse), Error>) in
            ...
            do {
                task = try PrivacyNetworkGate.uploadTask(with: request, from: file.data, using: session, feature: .localSend) { data, response, error in
                    if let error = error {

...
class ScreenshotSnippingTool: NSObject, ObservableObject {
    ...
    func startSnipping(type: ScreenshotType = .area, completion: @escaping (URL) -> Void) {
        guard Defaults[.enableScreenAssistant] else {
            print("🔒 ScreenshotTool: blocked because Screen Assistant is disabled")
            return
        }
    ...
    private func saveImageAndComplete(image: NSImage) {
        ...
        do {
            screenshotDir = try ScreenAssistantManager.ensureScreenshotDataDirectory()
        } catch {
            print("❌ ScreenshotTool: Failed to create screenshot directory: \(error)")
            finishSnipping()
            return
        }

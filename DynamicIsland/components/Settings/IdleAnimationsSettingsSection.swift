...
struct IdleAnimationsSettingsSection: View {
    ...
    @Default(.enableRemoteIdleAnimationURLs) private var enableRemoteIdleAnimationURLs
    ...
    var body: some View {
        Section {
            if showNotHumanFace {
                Defaults.Toggle(key: .enableRemoteIdleAnimationURLs) {
                    Text("Allow remote animation URLs")
                }
            ...
            if showNotHumanFace {
                Text("Choose animation to display when DynamicIsland is idle. Tap to select, hold to delete custom animations.")
            }
        }
        .fileImporter(
    ...
    private func handleURLImport() {
        guard enableRemoteIdleAnimationURLs else {
            importError = "Remote animation URLs are disabled in privacy-safe mode."
            showingError = true
            return
        }
...
struct AnimationPreview: View {
    ...
    var body: some View {
        ...
        case .lottieURL(let url):
            if enableRemoteIdleAnimationURLs {
                LottieView(state: LUStateData(
                    type: .loadedFrom(url),
                    speed: animation.speed,
                    loopMode: .loop

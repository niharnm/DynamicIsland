    ...
    private var providerStatusDescription: String {
        ...
        case .betterDisplay:
            if !betterDisplayManager.isDetected {
                return "Install [BetterDisplay](https://betterdisplay.pro) to control external display brightness (and optional volume) through DynamicIsland's HUD."
            }
            ...
            return "BetterDisplay OSD events will be routed through DynamicIsland's active HUD style. Brightness is always routed; volume is routed when external volume control listener is enabled below. Make sure BetterDisplay's OSD integration is enabled in Settings › Application › Integration."
        case .lunar:
            if !lunarManager.isDetected {
                return "Install [Lunar](https://lunar.fyi) to control external display brightness, contrast, and optional volume through DynamicIsland's HUD via DDC."
            }
            ...
            if lunarManager.isConnected {
                return "Connected to Lunar's DDC socket. Brightness and contrast adjustments are shown through DynamicIsland's HUD; volume follows when external volume control listener is enabled below."
            }
            ...
            Section {
                ...
                if enableThirdPartyDDCIntegration {
                    ...
                    Text(
                        ...
                        ? "DynamicIsland's built-in volume key interception is disabled while external volume listening is on. Volume HUD/OSD will follow \(thirdPartyDDCProvider.displayName) payloads."
                        : "DynamicIsland keeps native volume key interception. External provider volume payloads are ignored while this is off."
                    )
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    ...
                    Text("Enable to route BetterDisplay or Lunar display adjustments through DynamicIsland's active HUD style.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            } footer: {
                if enableThirdPartyDDCIntegration {
                    Text("DynamicIsland always listens to selected-provider brightness events, and listens to provider volume events only when external volume listener is enabled.")
                        .foregroundStyle(.secondary)
                        .font(.caption)
                }
            }
...
struct Media: View {
    ...
    @Default(.enableMusicMetadataLookups) private var enableMusicMetadataLookups
    @Default(.enableRemoteArtworkLoading) private var enableRemoteArtworkLoading
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Defaults.Toggle(key: .enableMusicMetadataLookups) {
                    Text("Allow online music metadata lookups")
                }
                Defaults.Toggle(key: .enableRemoteArtworkLoading) {
                    Text("Allow remote album artwork")
                }
...
struct CalendarSettings: View {
    ...
    var body: some View {
        ...
        .onChange(of: enableReminderLiveActivity) { _, enabled in
            guard enabled else { return }
            Task {
                await calendarManager.checkReminderAuthorization()
            }
        }
        .onChange(of: enableLockScreenReminderWidget) { _, enabled in
            guard enabled else { return }
            Task {
                await calendarManager.checkReminderAuthorization()
            }
        ...
        VStack {
            Form {
                ...
                HStack(spacing: 30) {
                    Spacer(minLength: 0)
                    Button {
                        NSWorkspace.shared.open(originalSourcePage)
                    } label: {
                        VStack(spacing: 5) {
                ...
                Text("Built over Atoll's open-source project by Ebullioscopic.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
...
struct Appearance: View {
    ...
    @Default(.enableRemoteIdleAnimationURLs) private var enableRemoteAnimationURLs
                ...
                .sheet(isPresented: $isPresented) {
                    VStack(alignment: .leading) {
                        ...
                        HStack {
                            ...
                            Button {
                                guard enableRemoteAnimationURLs,
                                      let visualizerURL = URL(string: url),
                                      (try? PrivacyNetworkGate.validate(visualizerURL, feature: .remoteIdleAnimation)) != nil
                                else {
                                    return
                                }
        ...
        Form {
            ...
            if enableClipboardManager {
                ...
                Section {
                    ...
                    Button("Clear All Clipboard Data") {
...
struct ScreenAssistantSettings: View {
    ...
    @Default(.enableExternalAIProviders) var enableExternalAIProviders
    ...
    var body: some View {
        Form {
            ...
            if enableScreenAssistant {
                Section {
                    Defaults.Toggle(key: .enableExternalAIProviders) {
                        Text("Allow cloud AI providers")
                    }
                    ...
                    .disabled(!enableExternalAIProviders)
                ...
                Section {
                    ...
                    Button("Clear Screen Assistant Data") {
                        screenAssistantManager.clearSensitiveLocalData()
                    }
                    .foregroundColor(.red)
                } header: {
                    Text("Actions")

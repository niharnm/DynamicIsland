    ...
    private var globalTogglesSection: some View {
        Section {
                ...
                if Defaults[.enableExtensionNotchExperiences] {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Defaults.Toggle(String(localized:"Allow interactive web content"), key: .enableExtensionNotchInteractiveWebViews)
                            .tint(.accentColor)
                        Defaults.Toggle(String(localized:"Allow extension web requests"), key: .enableExtensionRemoteRequests)
                            .tint(.accentColor)
                    }
                    .padding(.leading, 4)
                }
            ...
            if Defaults[.enableThirdPartyExtensions] {
                ...
                Text("Enable extensions to allow third-party apps to display live activities and lock screen widgets in DynamicIsland.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }

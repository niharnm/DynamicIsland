...
class AppleMusicController: MediaControllerProtocol {
    ...
    private func fetchArtworkFromCatalog(title: String, artist: String, album: String) async -> Data? {
        guard Defaults[.enableMusicMetadataLookups] else { return nil }
        ...
        do {
            let (data, _) = try await PrivacyNetworkGate.data(from: url, feature: .musicMetadata)
            ...
            let (imageData, _) = try await PrivacyNetworkGate.data(from: imageURL, feature: .remoteArtwork)

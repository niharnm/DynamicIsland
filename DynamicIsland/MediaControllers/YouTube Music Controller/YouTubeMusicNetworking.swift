...
final class YouTubeMusicHTTPClient: ObservableObject {
    ...
    func authenticate() async throws -> String {
        ...
        let (data, response) = try await PrivacyNetworkGate.data(for: request, using: session, feature: .youtubeMusic)
...
actor YouTubeMusicWebSocketClient {
    ...
    func connect(to url: URL, with token: String) async throws {
        ...
        var request = URLRequest(url: url)
        request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
        try PrivacyNetworkGate.validate(request, feature: .youtubeMusic)

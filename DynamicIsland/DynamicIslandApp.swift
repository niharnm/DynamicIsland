...
struct DynamicNotchApp: App {
    ...
    init() {
        Defaults.Keys.applyPrivacySafeDefaultsIfNeeded()
...
//    private var windowSizeUpdateWorkItem: DispatchWorkItem?
        ...
        -> NSWindow
    {
        ...
        window.contentView = FirstMouseHostingView(
            rootView: ContentView()
                ...
                .environmentObject(viewModel.webcamManager)
                //.moveToSky()
        )
    ...
    /// shadow insets and the configured vertical offset only when the screen
    /// lacks a physical notch and the user has chosen the Dynamic Island style.
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Setup Privacy Indicator Manager only when camera/microphone detection is explicitly enabled.
        ...
        Defaults.publisher(.enableScreenAssistant, options: []).sink { [weak self] _ in
            Task { @MainActor [weak self] in
                if !Defaults[.enableScreenAssistant] {
                    ScreenAssistantManager.shared.clearSensitiveLocalData()
                }
                self?.updateFeatureShortcutAvailability()
            }
        }.store(in: &cancellables)
        ...
        Defaults.publisher(.enableThirdPartyExtensions, options: []).sink { [weak self] _ in
            guard let self else { return }

            if Defaults[.enableThirdPartyExtensions] {
                self.extensionXPCServiceHost.start()
                self.extensionRPCServer.start()
            } else {
                self.extensionXPCServiceHost.stop()
                self.extensionRPCServer.stop()
            }
        }.store(in: &cancellables)
        ...
        .sink { change in
            if change.0.newValue || change.1.newValue {
                PrivacyIndicatorManager.shared.startMonitoring()
            } else {
        ...
        Defaults.publisher(.quickShareProvider, options: []).sink { change in
            if change.newValue != "LocalSend" {

...
struct ContentView: View {
    ...
    @State private var clickOpenSuppressionTask: Task<Void, Never>?
    @State private var clickOpenSuppressionToken: UUID?
...
//                .keyboardShortcut("E", modifiers: .command)
    ...
    private func MusicLiveActivity(secondary preResolvedSecondary: MusicSecondaryLiveActivity? = nil) -> some View {
        ...
        let standardNotchCompactWidthTarget = vm.closedNotchSize.width + (isHovering ? 8 : 0)
        let shouldKeepCompactMusicInsideMenuBarFootprint = (
            isNonNotchScreen &&
            !isDynamicIslandMode &&
            !inlineSneakPeekActive
        )
        let menuSafeCenterWidth = max(
            96,
            standardNotchCompactWidthTarget - wingBaseWidth - rightWingWidth
        )
        // Standard Notch on non-notched displays lives inside the menu bar, so
        // the closed music HUD should stay within the notch footprint instead
        // of widening into nearby app menus and status items.
        let effectiveCenterWidth = inlineSneakPeekActive
            ? 380
            : (shouldKeepCompactMusicInsideMenuBarFootprint ? menuSafeCenterWidth : centerBaseWidth)
    ...
    private func spectrumView(
        ...
        ZStack {
            if useMusicVisualizer || forceSpectrum {
                Rectangle()
                    .fill(compactMusicAccentColor.gradient)
    ...
    private var compactMusicAccentColor: Color {
        Defaults[.coloredSpectrogram]
            ? Color(nsColor: musicManager.avgColor)
            : Color(red: 0.18, green: 0.86, blue: 0.42)
    }
    ...
    private func openNotchFromUserAction() {
        holdOpenAfterClick()
        openNotch()
    }

    private func holdOpenAfterClick() {
        if let existingToken = clickOpenSuppressionToken {
            vm.setAutoCloseSuppression(false, token: existingToken)
        }
        clickOpenSuppressionTask?.cancel()

        let token = UUID()
        clickOpenSuppressionToken = token
        vm.setAutoCloseSuppression(true, token: token)

        clickOpenSuppressionTask = Task { @MainActor in
            try? await Task.sleep(for: .seconds(2))
            guard !Task.isCancelled else { return }
            vm.setAutoCloseSuppression(false, token: token)
            if clickOpenSuppressionToken == token {
                clickOpenSuppressionToken = nil
            }
        }
    }
    ...
    private func startHoverClickMonitor() {
        ...
        let handleClick: @Sendable () -> Void = { [weak vm, weak lockScreenManager] in
            Task { @MainActor in
                ...
                guard self.isHovering || self.isPointInsideNotchWindow(NSEvent.mouseLocation) else { return }
                guard !self.handleClosedMusicWaveformTapIfNeeded() else { return }
                if Defaults[.enableHaptics] {
                    self.triggerHapticIfAllowed()
                }
                self.openNotchFromUserAction()
            }
        }
...
private struct CompactMusicMetadataView: View {
    let title: String
    let artist: String
    let isPlaying: Bool
    ...
    private var displayArtist: String {
        ...
        return trimmed.isEmpty ? (isPlaying ? "Playing" : "Paused") : trimmed
    }

    var body: some View {
        HStack(spacing: 7) {
            Circle()
                .fill(isPlaying ? accentColor : Color.white.opacity(0.38))
                .frame(width: 6, height: 6)
                .shadow(color: isPlaying ? accentColor.opacity(0.55) : .clear, radius: 3, x: 0, y: 0)
        ...
        .accessibilityLabel("\(displayTitle), \(displayArtist)")
    }
}

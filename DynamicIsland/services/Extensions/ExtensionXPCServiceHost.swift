...
final class ExtensionXPCServiceHost: NSObject, NSXPCListenerDelegate {
    ...
    func start() {
        guard Defaults[.enableThirdPartyExtensions] else {
            Logger.log("XPC listener start blocked because third-party extensions are disabled", category: .extensions)
            return
        }
    ...
    func listener(_ listener: NSXPCListener, shouldAcceptNewConnection connection: NSXPCConnection) -> Bool {
        guard Defaults[.enableThirdPartyExtensions] else {
            Logger.log("Rejected XPC connection because third-party extensions are disabled", category: .extensions)
            return false
        }

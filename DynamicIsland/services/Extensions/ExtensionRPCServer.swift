...
final class ExtensionRPCServer {
    ...
    func start() {
        guard Defaults[.enableThirdPartyExtensions] else {
            logDiagnostics("RPC server start blocked because third-party extensions are disabled")
            return
        }
    ...
    private func handleNewConnection(_ nwConnection: NWConnection) {
        guard isLoopbackConnection(nwConnection) else {
            nwConnection.cancel()
            logDiagnostics("Rejected non-loopback RPC client")
            return
        }
    ...
    private func isLoopbackConnection(_ connection: NWConnection) -> Bool {
        ...
        case let .hostPort(host, _):
            ...
            case let .name(name, _):
                ...
                return normalized == "localhost" || normalized.hasPrefix("127.") || normalized == "::1"
